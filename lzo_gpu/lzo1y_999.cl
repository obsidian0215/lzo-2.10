/* lzo1y_999.cl -- GPU port of the LZO1Y-999 SWD compression algorithm
 *
 * True port of upstream lzo1y_9x.c Sliding Window Dictionary (SWD)
 * hash-chain algorithm to OpenCL. Produces byte-identical LZO1Y output
 * that can be decompressed by the standard lzo1y_decompress kernel.
 *
 * Design:
...
code_match_1y(__global uchar* op, __global uchar* out_start,
...
static __global uchar*
store_run_1y(__global uchar* op, __global uchar* out_start,
...
static __global uchar*
code_run_1y(__global uchar* op, __global uchar* out_start,
            compress_state_t* c,
            __global const uchar* ii, uint lit, uint m_len)
{
    if (lit > 0) {
        op = store_run_1y(op, out_start, ii, lit);
        c->r1_m_len = m_len;
        c->r1_lit = lit;
    } else {
        c->r1_m_len = 0;
...
static uint len_of_coded_match_1y(uint m_len, uint m_off, uint lit)
{
    uint n = 4;
...
static uint min_gain_1y(uint ahead, uint lit1, uint lit2,
...
static void better_match_1y(swd_state_t* s, uint* p_m_len, uint* p_m_off)
{
    uint m_len = *p_m_len;
    uint m_off = *p_m_off;
...
/* ================================================================
 * Main compression function — faithful port of lzo1y_999_compress_internal
 * ================================================================ */

static void
lzo1y_999_compress_block(__global const uchar* in, uint in_len,
                          ...
                          uint try_lazy_parm, uint max_chain_parm)
{
    ...
    swd.use_best_off = 1;  /* Enable best_off for better_match */

    /* Absolute position tracking:
     * ii_abs = absolute position in input of start of current literal run
     * bp_abs = absolute position of current match candidate
     */
    uint ii_abs = 0;
    ...
    while (look > 0) {
        ...
        if (swd.use_best_off)
            better_match_1y(&swd, &m_len, &m_off);
        ...
        if (try_lazy == 0 || m_len >= max_lazy) {
            ...
            l1 = len_of_coded_match_1y(m_len, m_off, lit);
        ...
        while (ahead < max_ahead && look > m_len) {
            ...
            if (swd.use_best_off)
                better_match_1y(&swd, &new_m_len, &new_m_off);

            l2 = len_of_coded_match_1y(new_m_len, new_m_off, lit + ahead);
            if (l2 == 0) continue;

            l3 = (op == out_start) ? 0 : len_of_coded_match_1y(ahead, m_off, lit);

            lazy_match_min_gain = min_gain_1y(ahead, lit, lit + ahead, l1, l2, l3);
            if (new_m_len >= m_len + lazy_match_min_gain) {
                /* Accept lazy match */
                if (l3) {
                    /* Code previous run + shortened match */
                    op = code_run_1y(op, out_start, &cc, in + ii_abs, lit, ahead);
                    lit = 0;
                    op = code_match_1y(op, out_start, &cc, ahead, m_off);
                } else {
                    lit += ahead;
                }
        ...
        op = code_run_1y(op, out_start, &cc, in + ii_abs, lit, m_len);
        lit = 0;

        op = code_match_1y(op, out_start, &cc, m_len, m_off);
        swd.max_chain = max_chain;
    ...
    if (lit > 0) {
        op = store_run_1y(op, out_start, in + ii_abs, lit);
    }
...
__kernel void lzo1y_block_compress_999(
...
)
{
    ...
    for (uint b = wi; b < total_blocks; b += total_wi) {
        ...
        lzo1y_999_compress_block(ip, in_len, op, &out_len[b],
