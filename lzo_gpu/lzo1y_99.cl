...
/*
 * LZO1Y Enhanced Greedy Kernel (Level 99 / 方案B)
 *
 * Enhancements over standard greedy (lzo1y.cl):
...
#define HASH_BUCKET(dv)  ((lzo1y_hash32(dv)) >> (32 - D_BITS))
...
static lzo_uint
lzo1y_99_compress_core(LZO_ADDR_GLOBAL const lzo_bytep in, lzo_uint in_len,
...
static __global uchar* lzo1y_99_compress_terminate(
                           ...
                           lzo_uint ti, __global uint *dict, uint epoch)
{
    ...
    t = lzo1y_99_compress_core(in, in_len, op, &olen, t, dict, epoch);
    op += olen;
    op = lzo1y_99_compress_terminate(in + in_len, op, t);

    *out_len = (lzo_uint)(op - out);
}

__kernel void lzo1y_block_compress_99(
...
/* -------- lzo1y decompress (identical to standard kernel) -------- */

#define M2_MAX_OFFSET_D 0x0400
