 ...
 * 从lzo_gpu_standalone.c提取,用于守护进程复用OpenCL资源
 *
...
/* Multi-threaded pread support (same pattern as lzo_gpu_standalone.c) */
typedef struct {
    int fd;
...
/* Asynchronous uploads were removed: synchronous blocking uploads are used
 * to simplify host-side buffer ownership and avoid complex reaper/threading
 * interactions which previously showed small net benefit and occasional regressions.
 */
...
static int write_compressed_file(const char* path,
                                 ...
                                 size_t stdio_buf_mb, int alg_id) {
    ...
    /* Configure stdio buffer size to reduce syscall noise from many small fwrite() calls.
     * Default: LZO_DEFAULT_STDIO_BUF_MB (4MB). This value is supplied per-request via
     * the request parameter. Clients may set the environment variable LZO_STDIO_BUF_MB
     * to define their preferred request default; the daemon does not read this env var
     * at initialization for global defaults.
     */
    int debug = getenv("LZO_DEBUG") != NULL;
    size_t comp_total = 0;
    for (size_t i = 0; i < nblk; i++) comp_total += lens[i];
    size_t vsize = stdio_buf_mb * 1024 * 1024;
    if (vsize == 0) vsize = LZO_DEFAULT_STDIO_BUF_MB * 1024 * 1024;
...
int daemon_compress(
    ...
    /* Use compile-time defaults from lzo_defaults.h. The daemon no longer
     * reads per-request tuning values from its own environment at startup.
     * Per-request values are honored via the client-provided request struct
     * (client-level env or CLI) and the daemon should only use header defaults
     * when the request leaves them unspecified (-1). We intentionally avoid
     * reading per-request env overrides at daemon init for simplicity.
     */
    ...
    /* async_upload removed; synchronous uploads only */
...
after_std_read:
         ...
         * Upload will be performed after lzo_choose_blocking_adaptive so that the
    ...
    size_t blk = 0, nblk = 0;
    size_t local_size = (local_size_param > 0) ? (size_t)local_size_param : 1; /* compression kernels require local=1 */
    if (local_size > 1) {
        fprintf(stderr, "[DAEMON] WARNING: Requested local_size=%zu for compression; forcing to 1 (compression kernel requires local=1).\n", local_size);
        local_size = 1;
    }
    ...
    {
        size_t fixed_blk_bytes = (fixed_block_kb > 0) ? (size_t)fixed_block_kb * 1024 : 0;
        lzo_choose_blocking_adaptive(entropy_ptr, in_sz, device, fixed_blk_bytes, &blk, &nblk, debug);
        if (debug) fprintf(stderr, "[DAEMON][DBG] lzo_choose_blocking_adaptive determined blk=%zu nblk=%zu (in_sz=%zu)\n", blk, nblk, in_sz);
    }
    ...
    size_t daemon_min_blk = MIN_BLOCK_SIZE;
    const char* daemon_min_env = getenv("LZO_MIN_BLOCK_SIZE");
    if (daemon_min_env && *daemon_min_env) {
        size_t tmp2 = lzo_parse_block_size(daemon_min_env);
        if (tmp2 > 0) daemon_min_blk = tmp2;
    }
    if (fixed_block_kb > 0) {
        size_t env_blk = (size_t)fixed_block_kb * 1024;
        if (debug) fprintf(stderr, "[DAEMON][DBG] request fixed_block_kb=%d env_blk=%zu daemon_min_blk=%zu\n", fixed_block_kb, env_blk, daemon_min_blk);
        if (env_blk > 0 && env_blk <= MAX_BLOCK_SIZE) {
            if (env_blk < daemon_min_blk) {
                fprintf(stderr, "[DAEMON] WARNING: Request fixed block size %zu KB is smaller than daemon min %zu KB; allowing override for this request\n", env_blk/1024, daemon_min_blk/1024);
            }
            if (debug) fprintf(stderr, "[DAEMON] 强制固定块大小 (request): %zu KB\n", env_blk / 1024);
            /* fixed blocks are honored by lzo_choose_blocking_adaptive; nothing to reassign here */
        }
    }
    ...
    if (!use_standard_copy) {
        ...
        if (mt_io && mt_threads > 1) {
            ...
            if (ev_count == 0) {
                ...
                if (debug) fprintf(stderr, "[DAEMON] write segments enqueued ev_count==0, skipping wait\n");
                upload_us = 0;
                free(evts);
                free(host_in);
                host_in = NULL;
    ...
    size_t lsz = LZO_LOCAL_SIZE_DEFAULT;
    ...
    */
    ...
    cl_event evt;
    err = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &gsz, &lsz, 0, NULL, &evt);
    CHECK(err);
    ...
    /* Asynchronous upload support removed; uploads were performed synchronously
     * and any host buffers were freed immediately after upload completion above.
     */
