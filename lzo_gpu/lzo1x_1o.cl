...
/* LZO1X-1O: 快速版本，使用15位字典 (D_BITS = 15) */
...
/* 快速版本：使用15位字典 (CPU标准实现) */
/* GPU优化: 保持15位字典以获得最佳压缩率 */
#define D_BITS          15
...
static lzo_uint
lzo1x_1o_compress_core(LZO_ADDR_GLOBAL const lzo_bytep in , lzo_uint  in_len,
    ...
    GINDEX(m_off,m_pos,in+dict,dindex,in);
        ...
        while (ip + m_len + 4 <= ip_end) {
            ...
            if (ip_val != mp_val) {
                ...
                m_len += (ctz(diff) >> 3);  /* ctz for little-endian */
                goto m_len_done;
            }
            m_len += 4;
        }
    ...
    lzo_uint  ti, lzo_voidp wrkmem)
{
    ...
    while (l > 20)
    {
        ...
        dict_clear(wrkmem);
        t = lzo1x_1o_compress_core(ip, ll, op, out_len, t, wrkmem);
        ip += ll;
        op += *out_len;
        l -= ll;
    }
