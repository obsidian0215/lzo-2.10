...
static inline void lzo_dbg_note_match_dist(__global uint* dbg_stats, uint dbg_base, uint dist)
{
    if (dist <= 64u) {
        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_DIST_LE_64, 1u);
    } else if (dist <= 256u) {
        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_DIST_LE_256, 1u);
    } else if (dist <= 1024u) {
        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_DIST_LE_1024, 1u);
    } else if (dist <= 4096u) {
        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_DIST_LE_4096, 1u);
    } else if (dist <= 16384u) {
        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_DIST_LE_16384, 1u);
    } else {
        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_DIST_GT_16384, 1u);
    }
}
                    ...
                    lzo_uint ti, __global uint *dict, uint epoch LZO_COMP_DBG_ARGS)
{
    ...
    literal:
            {
                lzo_uint skip = 1 + ((ip - ii) >> 5);
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_SKIP_ADVANCE_BYTES, skip);
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_SKIP_ADVANCE_OPS, 1u);
                ip += skip;
            }
    next:
            if (ip + 8 < ip_end)
            {
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_FAST_BATCH_ITERS, 1u);
                ...
                if (valid0 && off0 != 0) {
                    if (!(ip_off > off0 && (ip_off - off0) <= M4_MAX_OFFSET)) {
                        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_DISTANCE_FAIL, 1u);
                    } else {
                        m_pos = in + off0;
                        if (dvs_a.s0 == UA_GET_LE32(m_pos)) {
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_4B_MATCH, 1u);
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_LANE0, 1u);
                            LZO_DBG_COMP_NOTE_MATCH_DIST(dbg_stats, dbg_base, (uint)(ip_off - off0));
                        ...
                        goto match_found;
                            ...
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_4B_MISMATCH, 1u);
                        }
                    }
                }
                if (valid0 && off0 != 0) {
                ...
                if (valid1 && off1 != 0) {
                    if (!(ip_off > off1 && (ip_off - off1) <= M4_MAX_OFFSET)) {
                        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_DISTANCE_FAIL, 1u);
                    } else {
                        m_pos = in + off1;
                        if (dvs_a.s1 == UA_GET_LE32(m_pos)) {
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_4B_MATCH, 1u);
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_LANE1, 1u);
                            LZO_DBG_COMP_NOTE_MATCH_DIST(dbg_stats, dbg_base, (uint)(ip_off - off1));
                ...
                if (valid2 && off2 != 0) {
                    if (!(ip_off > off2 && (ip_off - off2) <= M4_MAX_OFFSET)) {
                        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_DISTANCE_FAIL, 1u);
                    } else {
                        m_pos = in + off2;
                        if (dvs_a.s2 == UA_GET_LE32(m_pos)) {
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_4B_MATCH, 1u);
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_LANE2, 1u);
                            LZO_DBG_COMP_NOTE_MATCH_DIST(dbg_stats, dbg_base, (uint)(ip_off - off2));
                ...
                if (valid3 && off3 != 0) {
                    if (!(ip_off > off3 && (ip_off - off3) <= M4_MAX_OFFSET)) {
                        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_DISTANCE_FAIL, 1u);
                    } else {
                        m_pos = in + off3;
                        if (dvs_a.s3 == UA_GET_LE32(m_pos)) {
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_4B_MATCH, 1u);
                            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_LANE3, 1u);
                            LZO_DBG_COMP_NOTE_MATCH_DIST(dbg_stats, dbg_base, (uint)(ip_off - off3));
                ...
                LZO_DBG_COMP_NOTE_STORE(dbg_stats, dbg_base, valid3, off3);
                dict_store32(dict, idx_a.s3, (uint)ip_off, epoch);
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_FAST_BATCH_FULL_MISS, 1u);
    ...
    next_slow:
            if (ip >= ip_end)
                break;
            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_SLOW_SINGLE_ITERS, 1u);
            dv = UA_GET_LE32(ip);
            lzo_uint ip_off = pd(ip, in);
            dindex = DINDEX(dv,ip);
            uint valid = 0;
            ...
            if (valid && m_off != 0) {
                if (ip_off > m_off && (ip_off - m_off) <= M4_MAX_OFFSET) {
                    ...
                    if (dv == UA_GET_LE32(m_pos)) {
                        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_4B_MATCH, 1u);
                        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_MATCH_SLOW_LANE, 1u);
                        LZO_DBG_COMP_NOTE_MATCH_DIST(dbg_stats, dbg_base, (uint)(ip_off - m_off));
                        saved_dindex = dindex;
                        goto match_found;
                    }
                    LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_VALID_4B_MISMATCH, 1u);
        ...
        if (t != 0)
        {
            ...
            if (t <= 3)
            {
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_LITERAL_SMALL_OPS, 1u);
            ...
            else
            {
                if (t <= 18)
                {
                    LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_LITERAL_SHORT_OPS, 1u);
                    *op++ = LZO_BYTE(t - 3);
                }
                else
                {
                    LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_LITERAL_LONG_OPS, 1u);
                    lzo_uint tt = t - 18;
                    *op++ = 0;
                    while (tt > 255)
                    {
                        LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_LITERAL_LONG_ZERO_RUNS, 1u);
                        tt -= 255;
                        UA_SET1(op, 0);
                        op++;
                    }
        ...
        while (ip + m_len + 32 <= ip_end) {
            ...
            if (diff0 != 0) {
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND_EXIT_32, 1u);
                m_len += (ctz(diff0) >> 3);
                goto m_len_done;
            }
            ...
            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND32_ITERS, 1u);
            m_len += 32;
        }
        while (ip + m_len + 8 <= ip_end) {
            ...
            if (ip_val != mp_val) {
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND_EXIT_8, 1u);
                ulong diff = ip_val ^ mp_val;
                m_len += (ctz(diff) >> 3);
                goto m_len_done;
            }
            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND8_ITERS, 1u);
            m_len += 8;
        }
#else
        while (ip + m_len + 8 <= ip_end) {
        ...
        while (ip + m_len + 4 <= ip_end) {
            ...
            if (ip32 != mp32) {
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND_EXIT_4, 1u);
                m_len += (ctz((ulong)(ip32 ^ mp32)) >> 3);
                goto m_len_done;
            }
            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND4_ITERS, 1u);
            m_len += 4;
        }
        while (ip + m_len < ip_end && ip[m_len] == m_pos[m_len]) {
            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND1_ITERS, 1u);
            m_len += 1;
        }
        if (ip + m_len < ip_end) {
            LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_CORE_EXTEND_EXIT_1, 1u);
        }
m_len_done:
            ...
            *op++ = LZO_BYTE(m_off >> 3);
            ...
            if (m_len <= M3_MAX_LEN)
                *op++ = LZO_BYTE(M3_MARKER | (m_len - 2));
                ...
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_M3_LONG_OPS, 1u);
                m_len -= M3_MAX_LEN;
                *op++ = M3_MARKER | 0;
                while(m_len > 255) {
                    LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_M3_LEN_ZERO_RUNS, 1u);
                    m_len -= 255;
                    UA_SET1(op, 0);
                    op++;
                }
            ...
            *op++ = LZO_BYTE(m_off >> 6);
            ...
            if (m_len <= M4_MAX_LEN)
                *op++ = LZO_BYTE(M4_MARKER | ((m_off >> 11) & 8) | (m_len - 2));
                ...
                LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_M4_LONG_OPS, 1u);
                m_len -= M4_MAX_LEN;
                *op++ = LZO_BYTE(M4_MARKER | ((m_off >> 11) & 8));
                while(m_len > 255) {
                    LZO_DBG_COMP_ADD(dbg_stats, dbg_base, LZO_DBG_COMP_M4_LEN_ZERO_RUNS, 1u);
                    m_len -= 255;
                    UA_SET1(op, 0);
                    op++;
                }
...
static void do_compress(__global const uchar* in, uint in_len, __global uchar* out, lzo_uintp out_len, lzo_uint ti, __global uint *dict, uint epoch LZO_COMP_DBG_ARGS)
{
    ...
    lzo_uint olen = 0;
    t = lzo1x_compress_core(in, in_len, op, &olen, t, dict, epoch LZO_COMP_DBG_PASS);
                                   ...
                                   )
{
    ...
    if (total_wi >= total_blocks) {
        if (wi >= total_blocks) return;
        uint b = wi;
...
#if LZO_GPU_DEBUG_COUNTERS_RUNTIME
        uint dbg_base = b * LZO_DBG_COMP_N;
...
__kernel void lzo1x_block_compress_range(__global const uchar *in,
                                         __global       uchar *out,
                                         __global       uint  *out_len,
                                         const uint  in_sz,
                                         const uint  blk_size,
...
static inline void lzo1x_fast_direct_match_copy_18(__global uchar *op,
    ...
    lzo_voidp wrkmem LZO_DEC_DBG_ARGS)
{
    ...
    *out_len = 0;
    ...
    for (;;) {
        ...
        if (t >= 16) goto match;
        if (t == 0) {
            LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_LITERAL_LONG_LEN, 1u);
            while (*ip == 0) { t += 255; ip++; }
            t += 15 + *ip++;
        }
        {
    ...
    first_literal_run:
        ...
        LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_M2_AFTER_LITERAL, 1u);
        LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_MATCH_LEN_LE4, 1u);
        ...
        *op++ = *m_pos;
        ...
        match:
            if (t >= 64) {
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_M2_MATCHES, 1u);
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_TOKEN_GE64, 1u);
                post_lit = t & 3;
                ...
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_TOKEN_32_63, 1u);
                t &= 31;
                if (t == 0) {
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_M3_LONG_LEN, 1u);
                    while (*ip == 0) {
                        t += 255; ip++;
                    }
                    t += 31 + *ip++;
                ...
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_TOKEN_16_31, 1u);
                m_pos = op - ((t & 8) << 11);
                t &= 7;
                if (t == 0) {
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_M4_LONG_LEN, 1u);
                    while (*ip == 0) {
                        t += 255;
                        ip++;
                    }
                ...
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_M2_MATCHES, 1u);
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_TOKEN_LT16_MATCH, 1u);
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_M2_SHORT2, 1u);
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_MATCH_LEN_LE4, 1u);
                post_lit = t & 3;
                m_pos = op - 1 - (t >> 2) - (*ip++ << 2);
                *op++ = *m_pos++;
        ...
        copy_match:
            {
                ...
                if (mlen <= 4u) {
                    ...
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_MATCH_LEN_LE8, 1u);
                } else if (mlen <= 16u) {
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_MATCH_LEN_LE16, 1u);
                } else {
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_MATCH_LEN_GT16, 1u);
                }
                if (offset >= mlen && mlen <= 18u) {
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_MATCH_OPS, 1u);
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_MATCH_BYTES, mlen);
                    if (offset <= 4) LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_SMALL_OFFSETS, 1u);
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_DIRECT_COPY, 1u);
                    lzo1x_fast_direct_match_copy_18(op, m_pos, mlen);
                } else {
                    LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_GENERIC_COPY, 1u);
        ...
        match_done:
            if (t == 0) {
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_POST_LIT0, 1u);
            } else if (t == 1) {
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_POST_LIT1, 1u);
            } else if (t == 2) {
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_POST_LIT2, 1u);
            } else {
                LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_POST_LIT3, 1u);
            }
            if (t == 0) break;
        match_next:
            LZO_DBG_DEC_ADD(dbg_stats, dbg_base, LZO_DBG_DEC_LITERAL_BYTES, t);
...
#endif
    lzo1x_decompress(in_buf + in_off, in_len, out_buf + out_off, &out_len, NULL LZO_DEC_DBG_PASS);
    if (out_lens) {
        out_lens[gid] = out_len;
    }
}

__kernel void lzo1x_block_decompress_range(
    __global const uchar* in_buf, __global const uint* off_arr,
    __global const uint* comp_lens,
    __global       uchar* out_buf, __global uint* out_lens,
