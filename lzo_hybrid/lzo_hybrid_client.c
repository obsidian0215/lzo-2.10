/*
 * lzo_hybrid_client.c - LZO GPU守护进程客户端
 *
...
/* client_socket_path is now provided by lzo_hybrid_protocol.h or lzo_hybrid_utils.h */
...
int decompress_with_daemon(const char* input, const char* output, int alg)
{
    ...
    if (!is_daemon_running()) {
        ...
        fprintf(stderr, "请先启动: ./lzo_hybrid --daemon\n");
        return -1;
    }
...
static void show_help(const char* prog) {
    ...
    fprintf(stderr, "注意: 需要先启动 ./lzo_hybrid --daemon 守护进程\n");
}
