...
#include <time.h>
/*
 * lzo_hybrid_daemon.c - LZO GPU守护进程实现
 *
 * 功能: 保持OpenCL上下文和缓冲区常驻内存,通过Unix socket接收压缩请求
 * 性能: 节省549ms/次的初始化开销 (OCL初始化44ms + 缓冲区分配505ms)
 *
 * 使用:
 *   启动守护进程: ./lzo_hybrid --daemon
 *   客户端请求:   ./lzo_hybrid --use-daemon <file>
 *   停止守护进程: ./lzo_hybrid --stop-daemon
 */
...
/* Decompress is implemented in lzo_hybrid_core: use lzo_decompress_core() */

#define SOCKET_PATH "/tmp/lzo_hybrid_daemon.sock"
#define PID_FILE "/tmp/lzo_hybrid_daemon.pid"
 ...
 */
/* file location helpers replaced by shared implementation in lzo_hybrid_utils.c */
...
int init_opencl_resources(void)
{
    ...
    for (int i = 0; i < MAX_WORKERS; i++) {
        ...
        lzo_hybrid_workspace_init(&g_state.workers[i].ws);
...
void cleanup_opencl_resources(void)
{
    ...
    for (int i = 0; i < MAX_WORKERS; i++) {
        ...
        lzo_hybrid_workspace_free(&g_state.workers[i].ws);
...
static int create_pidfile(void)
{
    ...
    if (flock(fd, LOCK_EX | LOCK_NB) != 0) {
        ...
        if (other_pid > 1) {
            ...
            if ((pf == NULL) || (strcmp(proc_comm, "lzo_hybrid") != 0)) {
...
static int open_default_logfile(void)
{
    ...
    if (!path || path[0] == '\0') {
        ...
        if (out_dir && out_dir[0] != '\0') {
            snprintf(buf, sizeof(buf), "%s/lzo_hybrid_daemon.log", out_dir);
            path = buf;
        } else {
            const char* tmpdir = getenv("TMPDIR");
            if (tmpdir && tmpdir[0] != '\0') {
                snprintf(buf, sizeof(buf), "%s/lzo_hybrid_daemon.log", tmpdir);
                path = buf;
            } else {
                path = "/tmp/lzo_hybrid_daemon.log";
            }
        }
    }
