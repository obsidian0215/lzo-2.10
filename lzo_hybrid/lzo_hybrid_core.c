/* lzo_hybrid_core.c - shared backend for compression/decompression
...
static size_t lzo_compress_slot_limit_for_device(cl_device_id device, size_t block_count, int level)
{
    cl_uint cus = 0;
    ...
    (void)clGetDeviceInfo(device, CL_DEVICE_TYPE, sizeof(dtype), &dtype, NULL);
    ...
    if (cus == 0) cus = 1;
    is_cpu = (dtype & CL_DEVICE_TYPE_CPU) ? 1 : 0;
    slots_env = is_cpu ? 0 : lzo_env_config()->gpu_comp_slots;

    int explicit_per_cu = per_cu_env > 0;
    if (per_cu_env <= 0) per_cu_env = LZO_OCC_FACTOR_DEFAULT;
    occ_cap = (size_t)cus * (size_t)per_cu_env;
    if (slots_env > 0) occ_cap = (size_t)slots_env;
    if (!explicit_per_cu && slots_env <= 0 && occ_cap < 1024U) occ_cap = 1024U;
    ...
    hard_slots = occ_cap < mem_cap ? occ_cap : mem_cap;
    if (target_items > hard_slots) target_items = hard_slots;
...
enum {
    ...
    LZO_DBG_COMP_FAST_BATCH_ITERS,
    LZO_DBG_COMP_SLOW_SINGLE_ITERS,
    LZO_DBG_COMP_VALID_DISTANCE_FAIL,
    LZO_DBG_COMP_VALID_4B_MISMATCH,
    LZO_DBG_COMP_VALID_4B_MATCH,
    LZO_DBG_COMP_CORE_EXTEND32_ITERS,
    LZO_DBG_COMP_CORE_EXTEND_EXIT_32,
    LZO_DBG_COMP_CORE_EXTEND_EXIT_8,
    LZO_DBG_COMP_CORE_EXTEND_EXIT_4,
    LZO_DBG_COMP_CORE_EXTEND_EXIT_1,
    LZO_DBG_COMP_LITERAL_SMALL_OPS,
    LZO_DBG_COMP_LITERAL_SHORT_OPS,
    LZO_DBG_COMP_LITERAL_LONG_OPS,
    LZO_DBG_COMP_LITERAL_LONG_ZERO_RUNS,
    LZO_DBG_COMP_M3_LONG_OPS,
    LZO_DBG_COMP_M4_LONG_OPS,
    LZO_DBG_COMP_M3_LEN_ZERO_RUNS,
    LZO_DBG_COMP_M4_LEN_ZERO_RUNS,
    LZO_DBG_COMP_MATCH_DIST_LE_64,
    LZO_DBG_COMP_MATCH_DIST_LE_256,
    LZO_DBG_COMP_MATCH_DIST_LE_1024,
    LZO_DBG_COMP_MATCH_DIST_LE_4096,
    LZO_DBG_COMP_MATCH_DIST_LE_16384,
    LZO_DBG_COMP_MATCH_DIST_GT_16384,
    LZO_DBG_COMP_MATCH_LANE0,
    LZO_DBG_COMP_MATCH_LANE1,
    LZO_DBG_COMP_MATCH_LANE2,
    LZO_DBG_COMP_MATCH_LANE3,
    LZO_DBG_COMP_MATCH_SLOW_LANE,
    LZO_DBG_COMP_FAST_BATCH_FULL_MISS,
    LZO_DBG_COMP_SKIP_ADVANCE_BYTES,
    LZO_DBG_COMP_SKIP_ADVANCE_OPS,
    LZO_DBG_COMP_N
};
...
enum {
    ...
    LZO_DBG_DEC_TOKEN_GE64,
    LZO_DBG_DEC_TOKEN_32_63,
    LZO_DBG_DEC_TOKEN_16_31,
    LZO_DBG_DEC_TOKEN_LT16_MATCH,
    LZO_DBG_DEC_M2_AFTER_LITERAL,
    LZO_DBG_DEC_M2_SHORT2,
    LZO_DBG_DEC_M3_LONG_LEN,
    LZO_DBG_DEC_M4_LONG_LEN,
    LZO_DBG_DEC_LITERAL_LONG_LEN,
    LZO_DBG_DEC_POST_LIT0,
    LZO_DBG_DEC_POST_LIT1,
    LZO_DBG_DEC_POST_LIT2,
    LZO_DBG_DEC_POST_LIT3,
    LZO_DBG_DEC_DIRECT_COPY,
    LZO_DBG_DEC_GENERIC_COPY,
    LZO_DBG_DEC_MATCH_LEN_LE4,
    LZO_DBG_DEC_MATCH_LEN_LE8,
    LZO_DBG_DEC_MATCH_LEN_LE16,
    LZO_DBG_DEC_MATCH_LEN_GT16,
    LZO_DBG_DEC_N
};
...
static void lzo_print_comp_debug_stats(const uint32_t* stats, size_t num_blocks, const char* tag) {
    ...
    unsigned long long fast_batch_iters = 0;
    unsigned long long slow_single_iters = 0;
    unsigned long long valid_distance_fail = 0;
    unsigned long long valid_4b_mismatch = 0;
    unsigned long long valid_4b_match = 0;
    unsigned long long core_extend32_iters = 0;
    unsigned long long core_extend8_iters = 0;
    unsigned long long core_extend4_iters = 0;
    unsigned long long core_extend1_iters = 0;
    unsigned long long core_extend_exit_32 = 0;
    unsigned long long core_extend_exit_8 = 0;
    unsigned long long core_extend_exit_4 = 0;
    unsigned long long core_extend_exit_1 = 0;
    unsigned long long literal_small_ops = 0;
    unsigned long long literal_short_ops = 0;
    unsigned long long literal_long_ops = 0;
    unsigned long long literal_long_zero_runs = 0;
    unsigned long long m3_long_ops = 0;
    unsigned long long m4_long_ops = 0;
    unsigned long long m3_len_zero_runs = 0;
    unsigned long long m4_len_zero_runs = 0;
    unsigned long long match_dist_le_64 = 0;
    unsigned long long match_dist_le_256 = 0;
    unsigned long long match_dist_le_1024 = 0;
    unsigned long long match_dist_le_4096 = 0;
    unsigned long long match_dist_le_16384 = 0;
    unsigned long long match_dist_gt_16384 = 0;
    unsigned long long match_lane0 = 0;
    unsigned long long match_lane1 = 0;
    unsigned long long match_lane2 = 0;
    unsigned long long match_lane3 = 0;
    unsigned long long match_slow_lane = 0;
    unsigned long long fast_batch_full_miss = 0;
    unsigned long long skip_advance_bytes = 0;
    unsigned long long skip_advance_ops = 0;
    ...
    for (size_t i = 0; i < num_blocks; ++i) {
        ...
        core_extend8_iters += stats[base + LZO_DBG_COMP_CORE_EXTEND8_ITERS];
        core_extend4_iters += stats[base + LZO_DBG_COMP_CORE_EXTEND4_ITERS];
        core_extend1_iters += stats[base + LZO_DBG_COMP_CORE_EXTEND1_ITERS];
        fast_batch_iters += stats[base + LZO_DBG_COMP_FAST_BATCH_ITERS];
        slow_single_iters += stats[base + LZO_DBG_COMP_SLOW_SINGLE_ITERS];
        valid_distance_fail += stats[base + LZO_DBG_COMP_VALID_DISTANCE_FAIL];
        valid_4b_mismatch += stats[base + LZO_DBG_COMP_VALID_4B_MISMATCH];
        valid_4b_match += stats[base + LZO_DBG_COMP_VALID_4B_MATCH];
        core_extend32_iters += stats[base + LZO_DBG_COMP_CORE_EXTEND32_ITERS];
        core_extend_exit_32 += stats[base + LZO_DBG_COMP_CORE_EXTEND_EXIT_32];
        core_extend_exit_8 += stats[base + LZO_DBG_COMP_CORE_EXTEND_EXIT_8];
        core_extend_exit_4 += stats[base + LZO_DBG_COMP_CORE_EXTEND_EXIT_4];
        core_extend_exit_1 += stats[base + LZO_DBG_COMP_CORE_EXTEND_EXIT_1];
        literal_small_ops += stats[base + LZO_DBG_COMP_LITERAL_SMALL_OPS];
        literal_short_ops += stats[base + LZO_DBG_COMP_LITERAL_SHORT_OPS];
        literal_long_ops += stats[base + LZO_DBG_COMP_LITERAL_LONG_OPS];
        literal_long_zero_runs += stats[base + LZO_DBG_COMP_LITERAL_LONG_ZERO_RUNS];
        m3_long_ops += stats[base + LZO_DBG_COMP_M3_LONG_OPS];
        m4_long_ops += stats[base + LZO_DBG_COMP_M4_LONG_OPS];
        m3_len_zero_runs += stats[base + LZO_DBG_COMP_M3_LEN_ZERO_RUNS];
        m4_len_zero_runs += stats[base + LZO_DBG_COMP_M4_LEN_ZERO_RUNS];
        match_dist_le_64 += stats[base + LZO_DBG_COMP_MATCH_DIST_LE_64];
        match_dist_le_256 += stats[base + LZO_DBG_COMP_MATCH_DIST_LE_256];
        match_dist_le_1024 += stats[base + LZO_DBG_COMP_MATCH_DIST_LE_1024];
        match_dist_le_4096 += stats[base + LZO_DBG_COMP_MATCH_DIST_LE_4096];
        match_dist_le_16384 += stats[base + LZO_DBG_COMP_MATCH_DIST_LE_16384];
        match_dist_gt_16384 += stats[base + LZO_DBG_COMP_MATCH_DIST_GT_16384];
        match_lane0 += stats[base + LZO_DBG_COMP_MATCH_LANE0];
        match_lane1 += stats[base + LZO_DBG_COMP_MATCH_LANE1];
        match_lane2 += stats[base + LZO_DBG_COMP_MATCH_LANE2];
        match_lane3 += stats[base + LZO_DBG_COMP_MATCH_LANE3];
        match_slow_lane += stats[base + LZO_DBG_COMP_MATCH_SLOW_LANE];
        fast_batch_full_miss += stats[base + LZO_DBG_COMP_FAST_BATCH_FULL_MISS];
        skip_advance_bytes += stats[base + LZO_DBG_COMP_SKIP_ADVANCE_BYTES];
        skip_advance_ops += stats[base + LZO_DBG_COMP_SKIP_ADVANCE_OPS];
    }
    ...
    fprintf(stderr,
            "%s hotpath fast_batch_iters=%llu slow_single_iters=%llu valid_distance_fail=%llu valid_4b_mismatch=%llu valid_4b_match=%llu core_extend32_iters=%llu core_extend8_iters=%llu core_extend4_iters=%llu core_extend1_iters=%llu core_extend_exit_32=%llu core_extend_exit_8=%llu core_extend_exit_4=%llu core_extend_exit_1=%llu\n",
            (tag ? tag : "[LZO-DBG][COMP]"),
            fast_batch_iters,
            slow_single_iters,
            valid_distance_fail,
            ...
            core_extend_exit_1);
    fprintf(stderr,
            "%s emit literal_small_ops=%llu literal_short_ops=%llu literal_long_ops=%llu literal_long_zero_runs=%llu m3_long_ops=%llu m4_long_ops=%llu m3_len_zero_runs=%llu m4_len_zero_runs=%llu\n",
            (tag ? tag : "[LZO-DBG][COMP]"),
            literal_small_ops,
            literal_short_ops,
            literal_long_ops,
            literal_long_zero_runs,
            m3_long_ops,
            m4_long_ops,
            m3_len_zero_runs,
            m4_len_zero_runs);
    fprintf(stderr,
            "%s probe_dist dist_le64=%llu dist_le256=%llu dist_le1024=%llu dist_le4096=%llu dist_le16384=%llu dist_gt16384=%llu lane0=%llu lane1=%llu lane2=%llu lane3=%llu slow_lane=%llu full_miss_batches=%llu skip_bytes=%llu skip_ops=%llu\n",
            (tag ? tag : "[LZO-DBG][COMP]"),
            match_dist_le_64,
            match_dist_le_256,
            match_dist_le_1024,
...
static void lzo_print_dec_debug_stats(const uint32_t* stats, size_t num_blocks, const char* tag) {
    ...
    unsigned long long token_ge64 = 0;
    unsigned long long token_32_63 = 0;
    unsigned long long token_16_31 = 0;
    unsigned long long token_lt16_match = 0;
    unsigned long long m2_after_literal = 0;
    unsigned long long m2_short2 = 0;
    unsigned long long m3_long_len = 0;
    unsigned long long m4_long_len = 0;
    unsigned long long literal_long_len = 0;
    unsigned long long post_lit0 = 0;
    unsigned long long post_lit1 = 0;
    unsigned long long post_lit2 = 0;
    unsigned long long post_lit3 = 0;
    unsigned long long direct_copy = 0;
    unsigned long long generic_copy = 0;
    unsigned long long match_len_le4 = 0;
    unsigned long long match_len_le8 = 0;
    unsigned long long match_len_le16 = 0;
    unsigned long long match_len_gt16 = 0;
    ...
    for (size_t i = 0; i < num_blocks; ++i) {
        ...
        token_ge64 += stats[base + LZO_DBG_DEC_TOKEN_GE64];
        token_32_63 += stats[base + LZO_DBG_DEC_TOKEN_32_63];
        token_16_31 += stats[base + LZO_DBG_DEC_TOKEN_16_31];
        token_lt16_match += stats[base + LZO_DBG_DEC_TOKEN_LT16_MATCH];
        m2_after_literal += stats[base + LZO_DBG_DEC_M2_AFTER_LITERAL];
        m2_short2 += stats[base + LZO_DBG_DEC_M2_SHORT2];
        m3_long_len += stats[base + LZO_DBG_DEC_M3_LONG_LEN];
        m4_long_len += stats[base + LZO_DBG_DEC_M4_LONG_LEN];
        literal_long_len += stats[base + LZO_DBG_DEC_LITERAL_LONG_LEN];
        post_lit0 += stats[base + LZO_DBG_DEC_POST_LIT0];
        post_lit1 += stats[base + LZO_DBG_DEC_POST_LIT1];
        post_lit2 += stats[base + LZO_DBG_DEC_POST_LIT2];
        post_lit3 += stats[base + LZO_DBG_DEC_POST_LIT3];
        direct_copy += stats[base + LZO_DBG_DEC_DIRECT_COPY];
        generic_copy += stats[base + LZO_DBG_DEC_GENERIC_COPY];
        match_len_le4 += stats[base + LZO_DBG_DEC_MATCH_LEN_LE4];
        match_len_le8 += stats[base + LZO_DBG_DEC_MATCH_LEN_LE8];
        match_len_le16 += stats[base + LZO_DBG_DEC_MATCH_LEN_LE16];
        match_len_gt16 += stats[base + LZO_DBG_DEC_MATCH_LEN_GT16];
    }
    ...
    fprintf(stderr,
            ...
            eof_markers);
    fprintf(stderr,
            "%s branch token_ge64=%llu token_32_63=%llu token_16_31=%llu token_lt16_match=%llu m2_after_literal=%llu m2_short2=%llu m3_long_len=%llu m4_long_len=%llu literal_long_len=%llu\n",
            (tag ? tag : "[LZO-DBG][DECOMP]"),
            token_ge64,
            token_32_63,
            token_16_31,
            ...
            literal_long_len);
    fprintf(stderr,
            "%s postlit post_lit0=%llu post_lit1=%llu post_lit2=%llu post_lit3=%llu direct_copy=%llu generic_copy=%llu match_len_le4=%llu match_len_le8=%llu match_len_le16=%llu match_len_gt16=%llu\n",
            (tag ? tag : "[LZO-DBG][DECOMP]"),
            post_lit0,
            post_lit1,
            post_lit2,
...
void lzo_hybrid_workspace_init(lzo_hybrid_workspace_t* ws) {
    memset(ws, 0, sizeof(lzo_hybrid_workspace_t));
    ws->comp_epoch_base = 1;
}

void lzo_hybrid_workspace_free(lzo_hybrid_workspace_t* ws) {
...
int lzo_compress_core(
    ...
    size_t dict_slots = lzo_compress_slot_limit_for_device(device, nblk, params->level);
    size_t target_items = dict_slots ? dict_slots : 1;
    ...
    uint32_t pool_size = (uint32_t)dict_slots;
...

    {
        ...
        size_t dict_pool_bytes = (size_t)pool_size * dict_per_block;
        size_t total_dict_size = dict_pool_bytes + sizeof(cl_uint);
        ...
        {
            cl_uint zero = 0;
            CHECK(clEnqueueWriteBuffer(queue, d_dict, CL_TRUE, dict_pool_bytes, sizeof(zero), &zero, 0, NULL, NULL));
        }
    ...
    unsigned long exec_us_event = 0;
    {
        ...
        clWaitForEvents(1, &ev);
        {
            ...
            if (clGetEventProfilingInfo(ev, CL_PROFILING_COMMAND_START, sizeof(st), &st, NULL) == CL_SUCCESS &&
                clGetEventProfilingInfo(ev, CL_PROFILING_COMMAND_END, sizeof(e), &e, NULL) == CL_SUCCESS) {
                exec_us_event = (unsigned long)((e - st) / 1000ULL);
            }
            if (debug_sched &&
                clGetEventProfilingInfo(ev, CL_PROFILING_COMMAND_QUEUED, sizeof(q), &q, NULL) == CL_SUCCESS &&
    ...
    *time_us_out = total_us; *output_size_out = comp_total;
    ...
    if (t_out) {
        ...
        t_out->kernel_exec_us = exec_us_event ? exec_us_event : exec_us_host;
    ...
    if (g_verbose) {
        /* verbose output is handled by lzo_hybrid.c wrapper to ensure
...
/* Free comp_host only if we allocated it (standard copy mode) */
...

    {
        ...
        kernel_has_dbg = dec_kernel_is_range ? (kernel_num_args >= 11U) : (kernel_num_args >= 10U);
        ...
        if (kernel_has_dbg) {
            ...
            cl_uint dbg_arg_index = dec_kernel_is_range ? 9U : 8U;
            CHECK(clSetKernelArg(kernel, dbg_arg_index, sizeof(cl_mem), &dbg_arg));
            CHECK(clSetKernelArg(kernel, dbg_arg_index + 1U, sizeof(cl_uint), &dbg_flag));
        }
    }
    ...
    uint64_t t_exec_start = core_now_ns();
    cl_event exec_event = NULL;
    err = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &global_size, &local_size, 0, NULL, &exec_event);
    CHECK(err);
    clWaitForEvents(1, &exec_event);
    uint64_t t_exec_end = core_now_ns();
    unsigned long exec_event_us = 0;
    if (exec_event) {
        cl_ulong st = 0, en = 0;
        if (clGetEventProfilingInfo(exec_event, CL_PROFILING_COMMAND_START, sizeof(st), &st, NULL) == CL_SUCCESS &&
            clGetEventProfilingInfo(exec_event, CL_PROFILING_COMMAND_END, sizeof(en), &en, NULL) == CL_SUCCESS) {
            exec_event_us = (unsigned long)((en - st) / 1000ULL);
        }
    ...
    {
        size_t chunk_threshold_kb = 16384U;
        size_t chunk_readback_kb = lzo_env_decomp_chunk_kb();
        int chunked_mode = lzo_env_decomp_chunked_mode();
        /*
         * Architecture policy:
         * - chunked readback/write is beneficial on Arc unified-memory path for large outputs;
         * - small outputs keep contiguous mapped write path to avoid extra readback overhead;
         * - policy remains overrideable through LZO_GPU_DECOMP_CHUNKED.
         */
        ...
        if (chunked_mode == 1) {
            use_chunked_output = output_path != NULL;
        } else if (chunked_mode == 0) {
            use_chunked_output = 0;
        } else {
    ...
    *output_size_out = orig_sz;
    ...
    if (t_out) {
        ...
        t_out->kernel_exec_us = exec_event_us ? exec_event_us : exec_host_us;
